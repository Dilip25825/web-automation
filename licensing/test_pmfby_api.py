...
class PmfbyApiTests(SimpleTestCase):
    ...
    def test_optout_request_uses_only_optout_service(self, select_record):
        select_record.return_value = (SimpleNamespace(
            ...
            for_whys='OptedOutForm', amount=2500, payment_status=0, is_active=1, entry_count=0,
        ), False)
        response = license_views.pmfby_subscription(self.post('pmfby_subscription', {
            'mobile': '9876543210', 'service': 'OptedOutForm', 'financial_year': 'Kharif 2026',
        }))
        data = json.loads(response.content)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['for_whys'], 'OptedOutForm')
        select_record.assert_called_once_with('9876543210', 'Kharif 2026', 'OptedOutForm')
        token_data = signing.loads(data['record_token'], salt=license_views.PMFBY_TOKEN_SALT)
        self.assertEqual(token_data['service'], 'OptedOutForm')
    ...
    def test_optout_years_fall_back_to_pmfby_when_not_configured(self, objects):
        ...
        self.assertEqual(license_views._pmfby_years('OptedOutForm'), ['Kharif 2026'])
        self.assertEqual(objects.filter.call_args_list[0].kwargs['forWhy__iexact'], 'OptedOutForm')
    ...
    def test_optout_options_use_optout_years(self, years):
        response = license_views.pmfby_options(self.post('pmfby_options', {'service': 'OptedOutForm'}))
        data = json.loads(response.content)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['for_whys'], 'OptedOutForm')
        years.assert_called_once_with('OptedOutForm')
    ...
    def test_optout_registration_creates_optout_record(self, _years, initial, service_queryset, objects, _atomic):
        initial.return_value = {'mobile': '9876543210', 'operator_mobile': '9876543210', 'service': 'OptedOutForm'}
        service_queryset.return_value.exists.return_value = False
        objects.create.return_value = SimpleNamespace(id=88)
        token = signing.dumps(
            {'mobile': '9876543210', 'service': 'OptedOutForm'},
